...
//     steps.push({ array: [...arr], comparedIndices: [] });
//     for (let i = 0; i < arr.length; i++) {
//       for (let j = 0; j < arr.length - i - 1; j++) {
//         if (arr[j] > arr[j + 1]) {
//           steps.push({ array: [...arr], comparedIndices: [j, j + 1] });
...
//           minIdx = j;
//         }
//         steps.push({ array: [...arr], comparedIndices: [j, minIdx] });
//       }
//       if (minIdx !== i) {
//         steps.push({ array: [...arr], comparedIndices: [i, minIdx] });
...
//       steps.push({ array: [...merged], comparedIndices: [] });
//       return merged;
//     };
...
//       if (arr[j] < pivot) {
//         steps.push({ array: [...arr], comparedIndices: [j, right] });
//         [arr[i], arr[j]] = [arr[j], arr[i]];
//         steps.push({ array: [...arr], comparedIndices: [] });
//         i++;
//       }
//     }
//     steps.push({ array: [...arr], comparedIndices: [i, right] });
...
//               sortingSteps[currentStep].array.map((num, idx) => (
//                 <div
//                   key={idx}
//                   className="mx-1 flex justify-center items-end rounded-t-md transition-all duration-300 ease-out"
//                   style={{
//                     backgroundColor: sortingSteps[currentStep].comparedIndices.includes(idx)
//                       ? "#FF0000" // Highlight color
//                       : "#3B82F6", // Default color
//                     width: `${Math.max(20, 80 / sortingSteps[0].array.length)}px`,
//                     height: `${num * 10}px`,
//                   }}
//                 >
...
//               {sortingSteps[sortingSteps.length - 1]?.array.map((num, idx) => (
//                 <span
//                   key={idx}
...
const SortingVisualizer = () => {
  ...
  const handleCompare = () => {
    ...
    const data = [];
    algorithms.forEach((algo) => {
      const arr = generateRandomArray(inputRange);
      const steps = [];
      switch (algo) {
        case "bubble-sort":
          bubbleSort([...arr], steps);
  ...
  return (
    <div className={`p-6 space-y-8 max-w-6xl mx-auto ${darkTheme ? "bg-gray-900" : "bg-gray-100"}`}>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg border-2 ${
            darkTheme 
              ...
              : "border-indigo-600 bg-indigo-400 hover:bg-indigo-500 text-gray-800"
      ...
      <div className="flex flex-col items-center space-y-4">
        <select
          className={`border-2 p-3 rounded-lg shadow-md w-64 ${
            darkTheme 
              ? "bg-gray-800 border-indigo-600 text-gray-200" 
        ...
        <input
          type="text"
          className={`border-2 p-3 rounded-lg shadow-md text-lg w-full md:w-96 ${
            darkTheme 
              ? "bg-gray-800 border-indigo-600 text-gray-200" 
              : "bg-white border-indigo-400 text-gray-700"
          } transition-colors duration-300`}
          placeholder="Enter numbers separated by commas (e.g., 5, 3, 8, 1)"
          value={arrayInput}
          onChange={(e) => setArrayInput(e.target.value)}
        ...
        <div className="flex gap-4 flex-wrap justify-center">
          <button
            className={`px-6 py-3 text-red rounded-lg shadow-md transition-all ${
              isSorting
          ...
          <button
            className="bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-lg shadow-md transition-all font-semibold"
            onClick={clearInput}
          >
            Clear
          </button>
          <button
            className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-lg shadow-md transition-all font-semibold"
            onClick={handleCompare}
          >
            Compare Algorithms
          </button>
      ...
      <div className="space-y-8">
        <div className={`p-6 rounded-xl shadow-lg ${
          darkTheme ? "bg-gray-800 border-indigo-600" : "bg-white border-indigo-400"
        } border-2 transition-colors duration-300`}>
          <h2 className={`text-2xl font-semibold mb-4 ${darkTheme ? "text-gray-200" : "text-gray-700"}`}>
            Sorting Visualization
          </h2>
          <div className={`flex justify-center items-end h-64 rounded-lg p-4 overflow-x-auto ${
        ...
        {hasSorted && !isSorting && (
          <div className={`p-6 rounded-xl shadow-lg ${
            darkTheme ? "bg-gray-800 border-emerald-600" : "bg-emerald-50 border-emerald-400"
          } border-2 transition-colors duration-300`}>
            <h3 className={`text-2xl font-semibold mb-4 ${darkTheme ? "text-gray-200" : "text-gray-700"}`}>
              Sorted Result
            </h3>
            <div className="flex flex-wrap gap-2">
              {sortingSteps[sortingSteps.length - 1]?.array.map((num, idx) => (
                <span
                  key={idx}
                  className="bg-emerald-500 text-white px-4 py-2 rounded-full shadow-md"
                >
                  {num}
                </span>
              ))}
        ...
        {showComparison && (
          <div ref={comparisonChartRef} className={`p-6 rounded-xl shadow-lg ${
            darkTheme ? "bg-gray-800 border-indigo-600" : "bg-white border-indigo-400"
          } border-2 transition-colors duration-300`}>
            <h2 className={`text-2xl font-semibold mb-4 ${darkTheme ? "text-gray-200" : "text-gray-700"}`}>
              Algorithm Comparison
            ...
            <Bar
              ...
              options={{
                ...
                scales: {
                  y: {
                    beginAtZero: true,
                    title: { text: "Steps", display: true, color: darkTheme ? "#E5E7EB" : "#1F2937" },
                    ticks: { color: darkTheme ? "#E5E7EB" : "#1F2937" },
                  },
                  x: {
                    ticks: { color: darkTheme ? "#E5E7EB" : "#1F2937" },
                  },
