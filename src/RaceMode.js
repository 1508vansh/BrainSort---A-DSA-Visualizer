...
const algorithmImplementations = {
  ...
  jump: function* ({ array, target }) {
    ...
    while (array[Math.min(step, array.length) - 1] < target) {
      prev = step;
      steps++;
      yield { progress: (steps / (array.length / step)) * 100 };
      if (prev >= array.length) break;
    }

    // Linear search phase
    while (prev < array.length && array[prev] <= target) {
      yield { progress: (prev / array.length) * 100 };
    ...
    // Final yield to complete progress
    yield { progress: 100 };
  },
...
const algorithmCategories = {
  ...
  searching: {
    ...
    //   jump: { name: 'Jump', complexity: 'O(√n)', color: 'bg-indigo-400' }
    }
  },
  pathfinding: {
    name: 'Pathfinding',
